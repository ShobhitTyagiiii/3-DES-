...
def encryptor(image_data, password):
    try:
        ...
        iv1, iv2, iv3 = os.urandom(8), os.urandom(8), os.urandom(8)

        cipher1 = DES.new(key[:8], DES.MODE_CBC, iv1)
        cipher2 = DES.new(key[8:16], DES.MODE_CBC, iv2)
        cipher3 = DES.new(key[16:], DES.MODE_CBC, iv3)

        encrypted_data = cipher3.encrypt(cipher2.decrypt(cipher1.encrypt(padded_data)))
...
def decryptor(encrypted_data_with_iv, password):
    try:
        ...
        decrypted_data = cipher1.decrypt(cipher2.encrypt(cipher3.decrypt(encrypted_data)))
